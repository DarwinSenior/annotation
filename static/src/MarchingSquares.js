...
(function(window) {
  ...
  MarchingSquares.getBlobOutlinePoints = function(sourceCanvas) {
    ...
    var i = 0;
    do{
      startingPoint = MarchingSquares.getFirstNonTransparentPixelTopDown(MarchingSquares.sourceCanvas);
      contours.push(MarchingSquares.walkPerimeter(startingPoint.x, startingPoint.y));
      this.floodfill(startingPoint.x, startingPoint.y, imageData);
      i++;
    }while(startingPoint || i<4);
  ...
  MarchingSquares.floodfill = function(startX, startY, imageData) {
    if (startX < 0) {
      startX = 0;
    }
    ...
    let width4 = imageData.width * 4;
    let index = (startY - 1) * width4 + (startX - 1) * 4;
    if (imageData.data[index + 3] > 0) {
      imageData.data[index + 3] = 0;
      this.floodfill(startX - 1, startY, imageData);
      this.floodfill(startX, startY - 1, imageData);
      this.floodfill(startX, startY + 1, imageData);
      this.floodfill(startX + 1, startY, imageData);
    }
  }
